...
if __name__ == '__main__':
    ...
    class QueryZoneDetail(tornado.web.RequestHandler):
        def set_default_headers(self):
            self.set_header("Access-Control-Allow-Origin", "*")

        def get(self, zone):
            query_result = collect_zone.find_one({"zone": zone}, {"_id": 0}) or {}
            self.write(query_result)
    ...
    if __name__ == "__main__":
        ...
        collect_zone = db[mongo["collect_zone"]]
        app = make_app()
        app.listen(8888)
